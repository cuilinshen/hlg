...
class HResponse extends HObject
{
    ...
    private static function _formatToImgLink($field, $record)
    {
        if('image_path' === $field) {
            ...
            if(strstr($value, 'http')) {
                return empty($value) ? HTranslate::__('没有图片') : '<a href="' . $value 
                . '" title="' . $imageName . '" class="lightbox" target="_blank"><img src="' 
                . $value . '" alt="' . $imageName . '"/></a>';    
            }
            return empty($value) ? HTranslate::__('没有图片') : '<a href="' . self::url() . $value 
                . '" title="' . $imageName . '" class="lightbox" target="_blank"><img src="' 
     ...
     * @param  $status 状态码
     */
    public static function alertModal($message, $url, $title = '温馨提醒您', $comfirmTitle = '点击确认跳转')
    {
        $url    = empty($url) ? HResponse::url() : $url;
        require_once(HResponse::path('tpl') . '/public/default/alert-modal.tpl');
     ...
     * @param  $status 状态码
     */
    public static function inputModal($id, $queryUrl,$url)
    {
        $url    = empty($url) ? HResponse::url() : $url;
        require_once(HResponse::path('tpl') . '/public/default/input-modal.tpl');
