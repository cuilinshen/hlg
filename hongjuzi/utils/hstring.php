...
class HString extends HObject
{
     ...
     * @return [type] [description]
     */
    public static function formatImage($imagePath, $type = '')
    {
        if(false === strpos($imagePath, 'http://') && false === strpos($imagePath, 'https://') ) {
            if(!$type) {
                return HResponse::url() . $imagePath;
            }
            $imageArr = explode('.', $imagePath);
            $imagePath= implode('.', array($imageArr[0] . '-' . $type, $imageArr[1]));
