...
class AdminAction extends AdministratorAction
{
    ...
    {
        parent::beforeAction();
        $totalNotice = HClass::quickLoadModel('notice')->getTotalRecords('`parent_id` < 2 AND `status` = 1');
        HSession::setAttribute('total_notice', $totalNotice, 'user');
        $loginTime      = date('Y-m-d H:i:s', HSession::getAttribute('login_time', 'user'));
        $totalOrders    = HClass::quickLoadModel('order')->getTotalRecords('`create_time` > \'' . $loginTime . '\'');
        HSession::setAttribute('total_orders', $totalOrders, 'user');       
    }
    ...
    protected function _otherJobsAfterList() 
    {
        ...
        $this->_assignShangMengCategoryList();
    ...
    protected function _assignAuthorInfo($author)
    {
        ...
        $user   = HClass::quickLoadModel('user')->getRecordById($author);
        HResponse::setAttribute('author', $user['name']);
    }
     ...
     * @return [type] [description]
     */
    protected function _assignShangMengCategoryList() 
    {
        HResponse::setAttribute(
            'shangmengCatList', 
            $this->_category->getSubCategoryByIdentifier('product-cat', false)
        );
    }
     ...
     * @return [type] [description]
     */
    protected function _getUserActor()
    {
        if(!HSession::getAttribute('parent_id', 'user')) {
            throw new HRequestException('对不起，您的权限不够');
        }

        return HClass::quickLoadModel('actor')->getRecordById(
