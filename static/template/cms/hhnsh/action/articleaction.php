 ...
 * @since 			1.0.0
 */
class ArticleAction extends HhnshAction 
{
     ...
     * 加载推荐新闻列表
     */
    protected function _assignRecommendNewsList($rows = 6)
    {
        $subCategoryList    = $this->_category->getSubCategoryByIdentifier('article-cat', true);
        $where              = '`is_recommend` = 2 AND ' . HSqlHelper::whereInByListMap('parent_id', 'id', $subCategoryList);
        $recommendNewsList  = $this->_model->getSomeRowsByFields($rows, '`id`, `name`, `image_path`, `author`, `create_time`, `description`', $where);
        
        HResponse::setAttribute('recommendNewsList', $recommendNewsList);
    }

    /**
     * 推荐产品列表
     */
    protected function _assignRecommendProductList()
    {
        $subCategoryList        = $this->_category->getSubCategoryByIdentifier('product-service', true);
        $where                  = '`is_recommend` = 2 AND ' . HSqlHelper::whereInByListMap('parent_id', 'id', $subCategoryList);
        $recommendProductList   = $this->_model->getSomeRowsByFields(5, '`id`, `name`', $where);
        
        HResponse::setAttribute('recommendProductList', $recommendProductList);
    }
     ...
     */
    protected function _setDetailInfo($record)
    {
        $userRecord     = HClass::quickLoadModel('user')->getRecordById($record['author']);
     ...
     * @return [type]         [description]
     */
    private function _assignCurDetailLocation($record)
    {
        $categoryRecord     = $this->_category->getRecordById($record['parent_id']);
        $parentPath         = explode(':', $categoryRecord['parent_path']);
        $curLocation        = array();
        foreach($parentPath as $key => $value) {
            if(!empty($value) && '447' != $value) {
                $curLocation[$value]  = $this->_category->getRecordById($value);
     ...
     * @access private
     */
    private function _assignCommentList()
    {
        $comment    = HClass::quickLoadModel('comment');
        HResponse::setAttribute(
            'commentList', 
            $comment->getAllRowsByFields(
    ...
    protected function _getRecommendArticleList($identifier, $rows = 5)
    {
        ...
        $articleList    = $this->_model->getSomeRowsByFields(
            $rows, '`id`, `name`, `description`, `image_path`, `edit_time`', 
            $where
        );
