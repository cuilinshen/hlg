...
/**
 * @version			$Id$
 * @create 			2013-11-04 22:11:10 By luodao
...
//导入引用文件
HClass::import('config.popo.resourcepopo, model.resourcemodel');
 ...
 * @since 			1.0.0
 */
class ResourceAction extends HhnshAction
{
    ...
    private function _aupload($extension, $isZoom = false)
    {
        ...
        if(!$fileInfo) {
            ...
            if(true === $isZoom) {
                $this->_zoomImage($fileInfo['path'], array('small' => array(120, 120), 'middle' => array(240, 240)));
            }
        }
    ...
    private function _addResourceLinkedData($file)
    {
        ...
        if($record) {
            throw new HVerifyException('文件已经存在，不用重复添加！');
        }
        $data           = array(
    ...
    public function adelete()
    {
        ...
        if(HRequest::getParameter('user_id') != $record['author']) {
